...
class VestingDetails extends Component {
  ...
  render() {
    ...
    const { start, cliff, end, total, released, vested, revocable, beneficiary, token } = this.props.details
    ...
    return <div className="details">
      ...
      <Table striped bordered condensed>
        <tbody>
          ...
          <TableRow title="Token">
            <ContractLink address={ token } />
          </TableRow>
  ...
  async onRelease() {
    ...
    try {
      this.startLoader()
      await tokenVesting.unlockVestedTokens({ from: accounts[0] })
      this.props.getData()
    } catch (e) {
      this.stopLoader()
    }
