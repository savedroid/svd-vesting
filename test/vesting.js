...
const expectRevert = assertRevert;
...
contract('SvdVesting', (accounts) => {
    var token;
    const owner = accounts[0];
    const assigner = accounts[1];
    const blocker = accounts[2];
    const investor = accounts[3];
    const vestedInvestor = accounts[4];
    const amount = 1000;
    ...
    beforeEach(async function () {
        ...
        trustee = await SvdVesting.new(token.address, {
            from: granter
        });
        //const result = await token.mint(trustee.address, amount);
    });
    ...
    describe('grant', async () => {
        ...
        context(`with ${balance} tokens assigned to the trustee`, async () => {
            ...
            it('should not allow granting to 0', async () => {
                await expectRevert(trustee.granting(0, 1000, now, now, now + 10 * YEAR, 1 * DAY, false));
            });

            it('should not allow granting to self', async () => {
                await expectRevert(trustee.granting(trustee.address, 1000, now, now, now + 10 * YEAR, 1 * DAY, false));
            });

            it('should not allow granting 0 tokens', async () => {
                await expectRevert(trustee.granting(accounts[0], 0, now, now, now + 3 * YEAR, 1 * DAY, false));
            });

            it('should not allow granting with a cliff before the start', async () => {
                await expectRevert(trustee.granting(accounts[0], balance, now, now - 1, now + 10 * YEAR, 1 * DAY, false));
            });

            it('should not allow granting with a cliff after the vesting', async () => {
                await expectRevert(trustee.granting(accounts[0], balance, now, now + YEAR, now + MONTH, 1 * DAY, false));
            });

            it('should not allow granting with 0 installment', async () => {
                await expectRevert(trustee.granting(accounts[0], balance, now, now + YEAR, now + MONTH, 0, false));
            });

            it('should not allow granting with installment longer than the vesting period', async () => {
                await expectRevert(trustee.granting(accounts[0], balance, now, now + YEAR, now + MONTH, 2 * YEAR, false));
            });

            it('should not allow granting tokens more than once', async () => {
                await trustee.granting(accounts[0], balance - 10, now, now, now + 10 * YEAR, 1 * DAY, false);

                await expectRevert(trustee.granting(accounts[0], 10, now, now, now + 10 * YEAR, 1 * DAY, false));
            });

            it('should not allow granting from not an owner', async () => {
                await expectRevert(trustee.granting(accounts[0], balance, now, now + MONTH, now + YEAR, 1 * DAY, false, {
                    from: accounts[1]
                }));
            });

            it('should not allow granting more than the balance in a single grant', async () => {
                await expectRevert(trustee.granting(accounts[0], balance + 1, now, now + MONTH, now + YEAR, 1 * DAY, false));
            });

            it('should not allow granting more than the balance in multiple grants', async () => {
                await trustee.granting(accounts[0], balance - 10, now, now + MONTH, now + YEAR, 1 * DAY, false);
                await trustee.granting(accounts[1], 7, now, now + MONTH, now + YEAR, 1 * DAY, false);
                await trustee.granting(accounts[2], 3, now, now + 5 * MONTH, now + YEAR, 1 * DAY, false);

                await expectRevert(trustee.granting(accounts[3], 1, now, now, now + YEAR, 1 * DAY, false));
            });
            ...
            it('should record a grant and increase grants count and total vesting', async () => {
                ...
                await trustee.granting(accounts[0], value, start, cliff, end, installmentLength, false);
                ...
                await trustee.granting(accounts[1], value2, start2, cliff2, end2, installmentLength2, false);
    ...
    describe('revoke', async () => {
        ...
        context(`with ${balance} tokens assigned to the trustee`, async () => {
            ...
            context('after minting has ended', async () => {
                ...
                it('should be able to revoke a revokable grant', async () => {
                    await trustee.granting(grantee, balance, now - MONTH, now - MONTH, now, 1 * DAY, true);

                    let balanceGranteeBefore = (await token.balanceOf(grantee)).toNumber()
                    let totalVestingBefore = (await trustee.totalVesting()).toNumber()

                    let result = await trustee.revoke(grantee);

                    let balanceGranteeAfter = (await token.balanceOf(grantee)).toNumber()
                    let totalVestingAfter = (await trustee.totalVesting()).toNumber()

                    console.log(balanceGranteeAfter, balanceGranteeBefore, totalVestingBefore, totalVestingAfter)

                    assert(balanceGranteeAfter > balanceGranteeBefore)
                    assert.equal(balanceGranteeAfter - balanceGranteeBefore, totalVestingBefore - totalVestingAfter)
                });

                it('should not be able to revoke a non-revokable grant', async () => {
                    await trustee.granting(grantee, balance, now, now + MONTH, now + YEAR, 1 * DAY, false);
                ...
                it('should only allow revoking a grant by an owner', async () => {
                    ...
                    await trustee.granting(grantee, balance, now, now + MONTH, now + YEAR, 1 * DAY, true);
                    await expectRevert(trustee.revoke(grantee, {
                        from: accounts[9]
                    }));

                    await trustee.revoke(grantee, {
                        from: granter
                    });
                });
            });
                    ...
                    `endOffset: ${grant.endOffset}, installmentLength: ${grant.installmentLength}`, async () => {
                        ...
                        for (let i = 0; i < grant.results.length; ++i) {
                            it(`should revoke the grant and refund tokens after ${i + 1} transactions`, async () => {
                                ...
                                await trustee.granting(holder, grant.tokens, now + grant.startOffset, now + grant.cliffOffset,
                                ...
                                for (let j = 0; j <= i; ++j) {
                                    ...
                                    await increaseTime(res.diff);
                                    await trustee.unlockVestedTokens({
                                        from: holder
                                    });
                                ...
                                assert.equal(userBalance3, userBalance2);//!!!user balance must have changed if there was an ongoing installment during revoke!!!
    ...
    describe('vestedTokens', async () => {
        ...
        [{
                ...
                results: [{
                    ...
                    {
                        offset: MONTH,
                        vested: Math.floor(1000 / 12)
                    },
                    {
                        offset: MONTH + 0.5 * DAY,
                        vested: Math.floor(1000 / 12) + Math.floor(0.5 * (1000 / 12 / 30))
                    },
                    {
                        offset: 2 * MONTH,
                        vested: 2 * Math.floor(1000 / 12)
                    },
                    {
                        offset: 0.5 * YEAR,
                        vested: 1000 / 2
                    ...
                    {
                        offset: 0.5 * YEAR + 3 * DAY,
                        vested: 1000 / 2 + Math.floor(3 * (1000 / 12 / 30))
                    },
                    {
                        offset: YEAR,
                        vested: 1000
            ...
            },
            {
                ...
                results: [{
                    ...
                    {
                        offset: MONTH + DAY,
                        vested: Math.floor(1000 / 12 + 1000 / 12 / 30)
                    },
                    {
                        offset: 2 * MONTH,
                        vested: 2 * Math.floor(1000 / 12)
                    ...
                    {
                        offset: 2 * MONTH + 1,
                        vested: Math.floor(2 * (1000 / 12))
                    },
                    {
                        offset: 2 * MONTH + 0.5 * DAY,
                        vested: Math.floor(2 * (1000 / 12))
                    },
                    {
                        offset: 2 * MONTH + 5 * DAY,
                        vested: Math.floor(2 * (1000 / 12) + 5 * (1000 / 12 / 30))
                    },
                    {
                        offset: 0.5 * YEAR,
                        vested: 1000 / 2
            ...
            },
            {
                ...
                results: [{
                    ...
                    {
                        offset: 2 * MONTH,
                        vested: 2 * Math.floor(10000 / 12 / 4)
                    },
                    {
                        offset: 0.5 * YEAR,
                        vested: 10000 / 8
            ...
            },
            {
                ...
                results: [{
                    ...
                    {
                        offset: YEAR + MONTH,
                        vested: Math.floor(10000 / 4 + 10000 / 4 / 12)
                    },
                    {
                        offset: YEAR + 2 * MONTH,
                        vested: Math.floor(10000 / 4 + 2 * (10000 / 4 / 12))
                    },
                    {
                        offset: YEAR + 3 * MONTH,
                        vested: Math.floor(10000 / 4 + 3 * (10000 / 4 / 12))
                    },
                    {
                        offset: 2 * YEAR,
                        vested: 10000 / 2
                    ...
                    {
                        offset: 3 * YEAR + MONTH,
                        vested: Math.floor(10000 * 0.75 + 10000 / 4 / 12)
                    },
                    {
                        offset: 3 * YEAR + 2 * MONTH,
                        vested: Math.floor(10000 * 0.75 + 2 * (10000 / 4 / 12))
                    },
                    {
                        offset: 3 * YEAR + 3 * MONTH,
                        vested: Math.floor(10000 * 0.75 + 3 * (10000 / 4 / 12))
                    }, {
                        offset: 4 * YEAR,
                        vested: 10000
                    },
            ...
            },
            {
                ...
                results: [{
                    ...
                    {
                        offset: MONTH,
                        vested: Math.floor(100000000 / 12 / 2)
                    },
                    {
                        offset: 2 * MONTH,
                        vested: Math.floor(2 * (100000000 / 12 / 2))
                    },
                    {
                        offset: 0.5 * YEAR,
                        vested: 100000000 / 4
            ...
            context(`grant: ${grant.tokens}, startOffset: ${grant.startOffset}, cliffOffset: ${grant.cliffOffset}, ` +
                `endOffset: ${grant.endOffset}, installmentLength: ${grant.installmentLength}`, async () => {
                    ...
                    beforeEach(async () => {
                        await trustee.granting(accounts[2], grant.tokens, now + grant.startOffset, now + grant.cliffOffset,
    ...
    describe('unlockVestedTokens', async () => {
        ...
        context('after minting has ended', async () => {
            ...
            it('should not allow unlocking a non-existing grant', async () => {
                ...
                await expectRevert(trustee.unlockVestedTokens({
                    from: holder
                }));
            });
            ...
            [{
                ...
                context(`grant: ${grant.tokens}, startOffset: ${grant.startOffset}, cliffOffset: ${grant.cliffOffset}, ` +
                    `endOffset: ${grant.endOffset}, installmentLength: ${grant.installmentLength}`, async () => {
                        ...
                        beforeEach(async () => {
                            await trustee.granting(holder, grant.tokens, now + grant.startOffset, now + grant.cliffOffset, now +
        ...
        it('should allow revoking multiple grants', async () => {
            ...
            for (let grant of grants) {
                await token.mint(trustee.address, grant.tokens);
                await trustee.granting(grant.holder, grant.tokens, now + grant.startOffset, now + grant.cliffOffset, now +
    ...
    describe('events', async () => {
        ...
        it('should emit events when granting vesting', async () => {
            let result = await trustee.granting(grantee, value, start, cliff, end, installmentLength, false);
        ...
        it('should emit events when revoking a grant', async () => {
            await trustee.granting(grantee, value, start, cliff, end, installmentLength, true);
            let result = await trustee.revoke(grantee);

            assert.lengthOf(result.logs, 3);

            let event = result.logs[2];
            assert.equal(event.event, 'GrantRevoked');
            assert.equal(event.args._holder, grantee);
            assert.equal(Number(event.args._refund), value);

            assert.equal(result.logs[0].event, 'TokensUnlocked');

            assert.equal(result.logs[1].event, 'TokensUnlocked');
        });

        it('should emit events when unlocking tokens', async () => {
            await trustee.granting(grantee, value, start, cliff, end, installmentLength, true);
            await increaseTime(cliff);
            let result = await trustee.unlockVestedTokens({
                from: grantee
            });
    ...
    describe('custom', async () => {
        ...
        it("should revoke", async function () {
            ...
            const toBeRefunded = 100; // = (amount / installmentLength) * 'number of installments after revoke call'

            assert.equal(0, (await trustee.totalVesting()).valueOf())
            assert.equal(token.address, await trustee.token())

            const res = await trustee.granting(
                investor,
                amount,
                now,
                cliff,
            ...
            assert.equal(amount, (await trustee.totalVesting()).valueOf());

            await increaseTimeTo(end - 10);
            const unl = await trustee.revoke(investor);
            const event = unl.logs.find(e => e.event === 'GrantRevoked');
            assert.equal(event.args._holder, investor);
            assert.equal(event.args._refund.valueOf(), toBeRefunded);
        });

        it("end to end", async function () {
            const cliff = now + 10;
            const end = now + 100;
            const installmentLength = 1;

            assert.equal(0, (await trustee.totalVesting()).valueOf())
            assert.equal(token.address, await trustee.token())
            // console.log(trustee.grant);
            ...
            assert.equal(0, (await trustee.vestedTokens(investor, cliff - 1)).valueOf())
            assert.equal((cliff - now) / (end - now) * amount, (await trustee.vestedTokens(investor, cliff)).valueOf())

            await increaseTimeTo(end)
            const unl = await trustee.unlockVestedTokens({
                from: investor
            })
            const event = unl.logs.find(e => e.event === 'TokensUnlocked')

            assert.equal(event.args._to, investor);
            assert.equal(event.args._value.valueOf(), amount)

            assert.equal(await token.balanceOf(investor), amount)
            assert.equal(await token.balanceOf(trustee.address), 0)
        })

        it("should compute correctly 1", async function () {
            const cliff = now + 360;
            const end = now + 370;
            const installmentLength = 130;
            const expectedVestedAmount = 702;
            ...
            const event = unl.logs.find(e => e.event === 'TokensUnlocked')
            assert.equal(event.args._to, investor);
            assert.equal(event.args._value.valueOf(), expectedVestedAmount)
        });

        it("should compute correctly 2", async function () {
            const cliff = now + 360;
            const end = now + 370;
            const installmentLength = 130;
            const expectedVestedAmount = 1000;
        ...
        it("should return nothing 1", async function () {
            const cliff = now;
            const end = now + 370;
            const installmentLength = 370;
            const expectedVestedAmount = 0;
            ...
            //console.log(JSON.stringify(unl))
            assert.equal(0, unl.logs.length)
        });



        it("should fail: cliff larger than end", async function () {
            const cliff = now + 380;
            const end = now + 370;
            const installmentLength = 130;
            const expectedVestedAmount = 702;
        ...
        it("should fail: cliff smaller than start", async function () {
            const cliff = now - 1;
            const end = now + 370;
            const installmentLength = 130;
            const expectedVestedAmount = 702;
        ...
        it("should fail: installmentLength larger than end-_start", async function () {
            const cliff = now + 360;
            const end = now + 370;
            const installmentLength = 371;
            const expectedVestedAmount = 702;
